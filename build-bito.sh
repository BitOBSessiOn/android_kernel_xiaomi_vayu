...
SECONDS=0
ZIPNAME="EtheriousB-LA.UM.9.x-vayu-$(date '+%Y%m%d-%H%M').zip"
...
mkdir -p out
make O=out ARCH=arm64 vayu_bito_defconfig

if [[ $1 == "-r" || $1 == "--regen" ]]; then
cp out/.config arch/arm64/configs/vayu_bito_defconfig
...
git clone -q https://github.com/xawlw/AnyKernel3
